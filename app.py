...
@app.before_request
def configure_session_cookie_policy():
    host = request.host.lower()
    origin = (request.headers.get("Origin") or "").lower()
    is_local = host.startswith(("127.0.0.1", "localhost")) or origin.startswith("http://127.0.0.1") or origin.startswith("http://localhost")
    if is_local:
